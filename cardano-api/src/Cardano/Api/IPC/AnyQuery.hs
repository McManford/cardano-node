  ...
  if ntcVersion >= minNtcVersion
    then do
      ...
      case res of
        ...
        Left eraMismatch -> left . AllQueryErrorSbe $ QueryEraMismatch  eraMismatch
        Right v -> return v
  ...
  if ntcVersion >= minNtcVersion
    then do
      ...
      case res of
        Left eraMismatch -> left . AllQueryErrorSbe $ QueryEraMismatch  eraMismatch
        Right v -> return v
    else left . AllQueryErrorSbe . SbeSimpleQueryError . SimpleQueryErrorUnsupportedVer
              $ UnsupportedNtcVersionError minNtcVersion ntcVersion
...
queryExprAnyQuery (AnyQueryShelleyBasedEra q) = do
  ...
  if ntcVersion >= minNtcVersion
    ...
    else left $ AllQueryErrorSbe $ SbeSimpleQueryError $ SimpleQueryErrorUnsupportedVer $ UnsupportedNtcVersionError minNtcVersion ntcVersion
...
determineEraInModeAnyQuery era cModeParams = do
  ...
  case toEraInMode era cMode of
    Nothing -> left . AllQueryErrorSbe $ EraInMode (anyCardanoEra era) (AnyConsensusMode cMode)
